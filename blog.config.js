...
const BLOG = {
  API_BASE_URL: process.env.API_BASE_URL || 'https://cooked-tree-e8d.notion.site/api/v3', // API默认请求地址,可以配置成自己的地址例如：https://cooked-tree-e8d.notion.site/api/v3
  // Important page_id！！！Duplicate Template from  https://tanghh.notion.site/02ab3b8678004aa69e9e415905ef32a5
  NOTION_PAGE_ID:
    process.env.NOTION_PAGE_ID ||
    'f7258c441794454cbdb97f035c297d1c',
  ...
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 21600, // 更新缓存间隔 单位(秒)；即每个页面有60秒的纯静态期、此期间无论多少次访问都不会抓取notion数据；调大该值有助于节省Vercel资源、同时提升访问速率，但也会使文章更新有延迟。
